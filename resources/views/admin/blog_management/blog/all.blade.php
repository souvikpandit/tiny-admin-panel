        ...
        <div class="card-body">
          <div class="tile mb30">
              <div class="tile-title-w-btn">
                  <div class="title">
                      <h3><i class="fe fe-24 fe-image"></i> Blog</h3>
                  </div>
                  @include('includes.message')
                  <ul class="list-inline">
                      <li class="list-inline-item"><button class="btn btn-danger btn-oval delete_all" data-url="{{ route('blogDeleteAll') }}"><i class="feather icon-trash"></i> Bulk Delete</button></li>
                      <li class="list-inline-item"><a href="{{ route('blog.create') }}" class="btn btn-primary btn-oval trigger-button"><i class="lni-plus"></i> Add New</a></li>
                  </ul>
              </div>
                      ...
                      <table id="myTable" class="table table-striped table-bordered dt-responsive nowrap page-table" cellspacing="0" width="100%">
                          ...
                          <tbody class="row_position">
                            @foreach ($blogs as $blog)
                              <tr id="tr_{{$blog->id}}" data-id="{{$blog->id}}">
                                <td>{{ $loop->iteration }}</td>
                                  <td>
                                    <input type="checkbox" class="sub_chk" data-id="{{$blog->id}}">
                                  </td>
                                  <td><img src="{{ asset(GetImageUrl($blog->image)) }}" alt="" style="width:50px"></td>
                                  <td>{{$blog->title}}</td>
                                  <td>
                                    @php
                                    $aid = $blog->author_id;
                                  ...
                                  <td>
                                      <a class="btn btn-sm btn-warning" href="{{ route('blog.edit',$blog->id) }}"><i class="feather icon-edit"></i> Edit</a>
                                      ...
                                      <a class="btn btn-sm btn-danger" href="" onclick="if(confirm('You Want to Delete This ?')){event.preventDefault();document.getElementById('delete-form-{{$blog->id}}').submit();}else{event.preventDefault();}"><i class="feather icon-trash"></i> Delete</a>
